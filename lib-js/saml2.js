...
create_authn_request = function(issuer, assert_endpoint, destination, force_authn, context, nameid_format, options) {
  var authnRequestPrefix, context_element, defaultElement, element, id, xml;
  if (options == null) {
    options = {};
  }
  if (context != null) {
  ...
  authnRequestPrefix = options.authnRequestPrefix || '';
  element = {};
  element[authnRequestPrefix + 'AuthnRequest'] = Object.assign(defaultElement, options.xmlAttributes);
  xml = xmlbuilder.create(element, options.xmlDec, options.xmlDoctype, options.xmlOptions).end();
  return {
    id: id,
    xml: xml
  };
...
module.exports.ServiceProvider = ServiceProvider = (function() {
  ...
  ServiceProvider.prototype.create_login_request_url = function(identity_provider, options, cb) {
    ...
    ref1 = create_authn_request(this.entity_id, this.assert_endpoint, identity_provider.sso_login_url, options.force_authn, options.auth_context, options.nameid_format, options), id = ref1.id, xml = ref1.xml;
